...
<div class="main">
    <form class="form-signin mt-5" v-bind:action="form_url" method="post" autocomplete='off'>
        <div class="text-right mb-5">
            <button v-if="value == 'login'" type="button" name="menue" class="btn btn-outline-info white" v-on:click="value_change" v-bind:value="value">{{ menue }}</button>
            <button v-else type="button" name="menue" class="btn btn-outline-success white" v-on:click="value_change" v-bind:value="value">{{ menue }}</button>
        </div>
        ...
        <!-- ログイン画面か登録画面かで色を変える -->
        <button v-if="value == 'login'" class="mt-5 btn btn-lg btn-primary btn-block" onclick="submit();" v-bind:class="class_name" type="button">{{ type }}</button>
        <button v-else class="mt-5 btn btn-lg btn-success btn-block" onclick="submit();" v-bind:class="class_name" type="button">{{ type }}</button>
        
        <p class="mt-5 mb-3 text-muted text-center">ユーザー{{ page }}</p>
    </form>
</div>
</template>
